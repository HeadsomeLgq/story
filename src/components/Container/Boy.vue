<template>
   <div>
     <div class="container">
       <h3 class="title">男生热文</h3>
       <div class="content" v-for="item in data.boy" :key="item.id">
     ...
     <div class="bookList" v-else>
       <div class="item" v-for="item in data.boy.slice(0,3)" :key="item.id">
...
<script>
    ...
    export default {
      name: 'Boy',
      methods: {
        ...mapActions([ 'receviveBoy',])
      },
      computed: {
        ...mapState(['data'])
      },
      created() {
        this.axios('/api/boy')  // 请求数据
          .then(res => {
